...
bool QLSqlTimeTableModel::InsertNewDatadb(int rowNo,QString TableName ,QStringList data)
{
        this->setTable(TableName);
        this->setEditStrategy(QSqlTableModel::OnManualSubmit);
        bool k = this->select();
        ...
        QSqlRecord tempRec = this->record(rowNo);
        int colCount = this->columnCount();
        for(int index = 0;index < colCount;index++)
        {
          //  qDebug() << tempRec;
            if(index == 0)
                tempRec.setValue(index ,rowNo);
            else
                tempRec.setValue(index ,data[index-1]);
        }

        k = this->insertRecord(rowNo,tempRec);
